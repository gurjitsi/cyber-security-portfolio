# TryHackMe — Learning Progress Log

**Profile:** [tryhackme.com/p/gurjit](https://tryhackme.com/p/gurjit.singh)
**Primary path:** Jr Penetration Tester  
**Started:** May 2026  
**Status:** 🔄 In progress

---

## Jr Penetration Tester Pathway

| Module | Topic | Status |
|--------|-------|--------|
| 1 | Start Your Cyber Security Journey | ✅ Completed |
| 2 | Introduction to Web Hacking | 🔄 In progress |
| 3 | Burp Suite | ⏳ Upcoming |
| 4 | Network Security | ⏳ Upcoming |
| 5 | Vulnerability Research | ⏳ Upcoming |
| 6 | Metasploit | ⏳ Upcoming |
| 7 | Privilege Escalation | ⏳ Upcoming |

---

## Key Learnings Log

### Week 1 — May 2026
- Set up TryHackMe account and OpenVPN connection
- Completed intro to pentesting methodology
- Key learning: reconnaissance → scanning → exploitation → post-exploitation → reporting

---

## Rooms Completed

| Room | Category | Difficulty | Notes |
|------|----------|------------|-------|
| *Update as you complete rooms* | | | |

---

## Tools Encountered

| Tool | Purpose | Notes |
|------|---------|-------|
| Nmap | Network scanning | Port discovery, service enumeration |
| Burp Suite | Web proxy | Intercepting HTTP requests |
| Gobuster | Directory enumeration | Finding hidden web paths |
| Hydra | Brute forcing | Login attack tool |

---

*Updated weekly as I progress through the pathway.*
